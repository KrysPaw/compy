# Todo

Quick notes for improvements noticed while using or working on Compy.
Move items into a feature plan or `REMAINING.md` when they become real work.

## Open

- Mobile card/list layouts for comparison tables (Entries / Criteria / Rules / Results)
- Refactor dialogs
- Improve missing `RESEND_API_KEY` handling (fail clearly in production instead of console-only magic-link fallback)

## Done

- Make app shell / comparison chrome responsive for mobile
- Add logout
- Refactor sidebar bottom section
- Add shared comparisons list below "My comparisons" in UI
- Translate mail content for different languages
- Key criterion default name based on language
